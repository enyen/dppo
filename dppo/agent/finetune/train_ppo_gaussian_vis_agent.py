"""
PPO training for Gaussian/GMM policy with pixel or pointcloud observations.
...
from dppo.agent.finetune.train_ppo_gaussian_agent import TrainPPOGaussianAgent
from dppo.model.common.modules import RandomShiftsAug


class TrainPPOVisGaussianAgent(TrainPPOGaussianAgent):

    def __init__(self, cfg):
        super().__init__(cfg)

        # visual key
        self.vis_key = cfg.train.vis_key
        ...
        while self.itr < self.n_train_itr:
            ...
            if not eval_mode:
                with torch.no_grad():
                    ...
                    obs_trajs[self.vis_key] = (
                        torch.from_numpy(obs_trajs[self.vis_key]).float().to(self.device)
                    )
                    obs_trajs["state"] = (
                    ...
                    if self.augment:
                        ...
                        vis = self.aug(vis)
                        obs_trajs[self.vis_key] = einops.rearrange(
                            vis,
                            "(s e t) c h w -> s e t c h w",
                            s=self.n_steps,
                            e=self.n_envs,
